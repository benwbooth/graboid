...
async fn api_test_llm(
    ...
    let model = form
        .get("llm_model")
        .cloned()
        .unwrap_or_else(|| state.config.claude_model.clone());
            ...
            .await
        {
            Ok(resp) if resp.status().is_success() => {
                let count = resp
                    ...
                    .unwrap_or(0);
                return Ok(Json(
                    json!({"success": true, "message": format!("Ollama connected. {count} models available.")}),
                ));
            }
            Ok(resp) => {
                return Ok(Json(
                    json!({"success": false, "error": format!("HTTP {}", resp.status())}),
                ));
            }
            Err(err) => {
                return Ok(Json(json!({"success": false, "error": err.to_string()})));
    ...
    if let Some(env_name) = key_map.get(provider.as_str()) {
        if std::env::var(env_name).is_ok() {
            return Ok(Json(json!({
                "success": true,
                "message": format!("{provider} API key found ({env_name})")
            })));
        }
        return Ok(Json(json!({
            "success": false,
            "error": format!("Missing {env_name} environment variable")
        })));
    }
...
async fn api_ollama_models(State(state): State<Arc<AppState>>) -> Result<Json<Value>, ApiError> {
    let url = format!("{}/api/tags", state.config.ollama_host);
